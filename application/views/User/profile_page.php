		...
		<table id="example" class="table table-hover table-striped " style="margin-top: 20px; border-bottom: 1px solid Steelblue">
			...
			<tbody >
				...
				<tr>
					...
					<td class="text-capitalize"><?= $key['fname'].' '.$key['lname']?></td>
					<td class="text-capitalize"><?php echo $key['username']; ?></td>
					<td class="text-capitalize"><?php echo $key['sex']; ?></td>
					<td ><a href="#"><?php echo $key['email_add']; ?></a></td>
					<td ><?php echo $key['date_reg']; ?></td>
					<td>
						<button class="btn btn-info btn-xs" type="button" onclick="window.location='<?= base_url();?>Login/update_admin/<?php echo $key['admin_id']; ?>';"><span class="glyphicon glyphicon-pencil"></span></button>
						<a href="<?php echo base_url();?>Control/delete_admin/<?php echo $key['admin_id']; ?>" class="btn btn-danger btn-xs glyphicon glyphicon-trash" onclick="return confirm('Are you sure?')"></a>
					</td>
				</tr>
				<?php endforeach ?>
			</tbody>
