...
<header style="background: url('<?= base_url() ?>/assets/images/EVSU_banner.png'); height: 120px; width: 100%" class="img-responsive" alt="EVSU | College of Engineering | On the Job Training Monitoring and Grading System">	
</header>
						...
						<div class="">	
							<form class="form-horizontal" action="<?php echo base_url();?>Control/register_official_stud_user" method="POST">	
									<div class="form-group">	
										...
										<div class="col-sm-8">
											<input type="text" name="reg_stud_num" class="form-control text-capitalize" placeholder="Student number" minlength="10" maxlength="10" autofocus required>
...
<div class="table_list table-responsive" style="background: #fff; border-radius: 2px; padding: 5px 15px 15px 15px; box-shadow: 0 1px 2px 1px rgba(0, 0, 0, 0.2), 0 1px 2px 0 rgba(0, 0, 0, 0.1);">
	...
	<div id="" class="">
	<div class="">
		<h1 align="left"><span class="fa fa-users"></span> List of Students Grades</h1>
	</div>
		...
		<table id="example" class="table table-striped table-hover" cellspacing="0" style="margin-top: 10px;border-bottom: 1.5px solid SteelBlue;">
			<thead>
				<tr style="color: #fff; background: SteelBlue">
					...
					<th>Trainee Supervisor's Grade </th>
			...
			<tbody>
			<?php foreach ($this->Login_user_model->get_all_student_grades($course_id) as $key): ?>
				<tr>
					<td class="text-capitalize"><?= $key['year'].'-'.$key['section']?></td>
					<td>
						<a href="<?php echo base_url();?>Login/student_profiles/<?php echo $key['stud_id'] ?>" style="color: #1565c0"> <?= $key['stud_id']; ?></a>
					</td>
					<td class="text-capitalize"><input type="hidden" name="fullname"><?php echo $key['lname'].' '.$key['fname']; ?></td>
					<td class="text-capitalize" align="center"><a href="#"><input type="hidden" name="cdr_grade"><?=$key['cdr_grade']?></a></td>
					<td class="text-capitalize" align="center"><a href="#"><input type="hidden" name="spv_grade"><?=$key['spv_grade']?></a></td>
					<td class="text-capitalize" align="center"><a href="#"><input type="hidden" name="PTPgrade"><?=$key['PTPgrade']?></a></td>
