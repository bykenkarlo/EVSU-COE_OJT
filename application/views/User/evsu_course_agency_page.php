...
<div class="col-sm-12 " >
	...
	<div style="margin-bottom: 10px;" class="btn-group">
		...
		<button type="button" class="btn btn-info btn-lg" onclick="window.location='<?= base_url();?>Login/coordinator_lists'"><span class="fa fa-th-list"></span> Course Lists</button>
	</div>
		...
		<table id="example" class="table table-hover table-striped " style="margin-top: 20px; border-bottom: 1px solid Steelblue">
			...
			<tbody >
				...
				<tr>
					...
					<td>
						<button class="btn btn-info btn-xs" type="button" onclick="window.location='<?= base_url();?>Login/update_admin/<?php echo $key['course_id']; ?>';"><span class="glyphicon glyphicon-pencil"></span></button>
						<a href="<?php echo base_url();?>Control/delete_admin/<?php echo $key['course_id']; ?>" class="btn btn-danger btn-xs glyphicon glyphicon-trash" onclick="return confirm('Are you sure?')"></a>
					</td>
				</tr>
				<?php endforeach ?>
			</tbody>
